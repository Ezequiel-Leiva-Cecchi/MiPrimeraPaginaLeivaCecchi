...
class CineVaultTests(TestCase):
    def setUp(self):
        ...
        self.pelicula = Pelicula.objects.create(titulo="Inception", fecha_lanzamiento=date(2010, 7, 16), mini_resumen="Un ladrón entra en los sueños.", director=self.director)
    ...
    def test_catalogo_y_detalle_son_publicos(self):
        ...
        self.assertContains(self.client.get(reverse("detalle_pelicula", args=[self.pelicula.pk])), "Inception")

    def test_busqueda_por_director_y_genero(self):
        response = self.client.get(reverse("buscar_pelicula"), {"q": "Nolan", "genero": self.genero.pk})
    ...
    def test_lista_personal_agrega_y_elimina_sin_duplicar(self):
        ...
        self.assertTrue(EnLista.objects.filter(usuario=self.user, pelicula=self.pelicula).exists())
        self.client.post(url)
        self.assertFalse(EnLista.objects.filter(usuario=self.user, pelicula=self.pelicula).exists())
